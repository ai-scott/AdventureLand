...
```
scripts/systems/ui/
├── button-manager.ts          # Core button pooling/lifecycle
├── notification-manager.ts    # Notification queue and priority
├── button-actions.ts          # Action registration and routing
├── ui-types.ts               # Shared TypeScript interfaces
├── claude.md                 # System documentation
└── README.md                 # Integration guide

tests/systems/
└── ui-button-manager.test.ts # Comprehensive test suite
...
### 2. Notification Manager (`notification-manager.ts`)
...
### Phase 1: Core Button Manager (Week 1)

**Goal**: Eliminate manual button creation/destruction
...
4. Add namespace exposure in `main.ts`
5. Write basic tests in `ui-button-manager.test.ts`
...
**Goal**: Solve Bug #9 (first item pickup notification)

**Tasks**:
1. Implement `notification-manager.ts`
2. Add queue and priority system
3. Implement auto-dismiss logic
4. Add "show once" tracking
5. Create notification tests

**Deliverables**:
- `showNotification()` working
- Priority queue prevents overlaps
- Auto-dismiss after timeout
- History tracking for tutorials

**New Feature**: First item pickup notification
```javascript
// When player picks up first item
notifMgr.showNotification({
  id: "first-item-pickup",
  type: "tutorial",
  message: "Press 'i' to open inventory",
...
- Bug #9 resolved
- No hint overlaps
- Notifications queue gracefully
- Performance: <1% CPU overhead

### Phase 3: Action Registry (Week 3)

**Goal**: Decouple button actions from button UI

**Tasks**:
1. Implement `button-actions.ts`
2. Register existing actions (Inventory, Attack)
...
- Inventory and Attack actions migrated
...
This UI Button System will:

✅ **Solve immediate problems**:
- Bug #9 (first item notification)
- Fragile hint system
- Complex event sheet logic
...
- Pooling (inventory UI)
- Bridge (dialogue system)
- Factory (enemy AI)
...
**Estimated Impact**:
- 80%+ reduction in event sheet complexity
- 90%+ reduction in development time for new buttons
- <1% CPU overhead
- Bug #9 resolved
