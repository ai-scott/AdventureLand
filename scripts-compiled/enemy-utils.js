...
// ===== MOVEMENT FUNCTIONS =====
export function moveTowardPlayer(behavior8Dir, enemy, enemyData, speed, runtime) {
...
export function moveAwayFromPlayer(behavior8Dir, enemy, enemyData, speed, runtime) {
    const player = getPlayerInstance(runtime);
    if (!player) {
        behavior8Dir.maxSpeed = 0;
        return;
...
export function moveCrabTowardPlayer(behavior8Dir, enemy, enemyData, speed, runtime) {
    const player = getPlayerInstance(runtime);
    if (!player) {
        behavior8Dir.maxSpeed = 0;
        return;
...
export function moveSideways(behavior8Dir, enemy, enemyData, speed, direction, runtime) {
    const player = getPlayerInstance(runtime);
    if (!player) {
        behavior8Dir.maxSpeed = 0;
        return;
...
export function moveRandomly(behavior8Dir, enemyData, speed) {
    if (Math.random() < 0.1) {
        behavior8Dir.maxSpeed = speed;
...
// ===== ANIMATION FUNCTIONS =====
export function executeAnimation(enemy, enemyData, animationName, runtime) {
    try {
        if (!animationName)
            return;
...
// ===== Z-ORDERING SYSTEM =====
export function updateEnemyZOrder(enemy, runtime) {
    if (!enemy || !runtime)
        return;
    try {
...
// ===== UTILITY FUNCTIONS =====
export function calculateDirection(enemy, player) {
    const dx = player.x - enemy.x;
    const dy = player.y - enemy.y;
    if (Math.abs(dx) > Math.abs(dy)) {
...
export function getRandomDuration(duration) {
    return duration[0] + Math.random() * (duration[1] - duration[0]);
}
export function evaluateCondition(currentValue, operator, targetValue) {
    switch (operator) {
...
// ===== INSTANCE GETTERS =====
export function getPlayerInstance(runtime) {
    if (!runtime)
        return null;
    try {
...
export function getEnemyInstance(baseUID, runtime) {
    if (!runtime)
        return null;
    try {
        const objectTypes = [
        ...
        for (const objectType of objectTypes) {
            if (objectType && typeof objectType.getPickedInstances === 'function') {
                ...
                if (instances) {
                    const instance = instances.find((inst) => inst && inst.uid === baseUID);
                    if (instance)
                        return instance;
                }
            }
...
export function getMaskInstance(maskUID, runtime) {
    if (!runtime)
        return null;
    try {
        ...
        for (const objectType of objectTypes) {
            if (objectType && typeof objectType.getPickedInstances === 'function') {
                const instances = objectType.getPickedInstances();
                if (instances) {
                    const instance = instances.find((inst) => inst && inst.uid === maskUID);
                    if (instance)
                        return instance;
                }
            }
...
//# sourceMappingURL=enemy-utils.js.map