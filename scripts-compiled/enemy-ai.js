...
import { getEnemyConfig } from "./enemy-configs.js";
import { moveTowardPlayer, moveAwayFromPlayer, moveCrabTowardPlayer, moveSideways, moveRandomly, executeAnimation, calculateDirection, getRandomDuration, evaluateCondition, getPlayerInstance, getEnemyInstance } from "./enemy-utils.js";
// ===== ENEMY AI FACTORY =====
export class EnemyAIFactory {
    ...
    initEnemy(baseUID, maskUID, enemyType, config) {
    ...
    initializeMovementBehavior(baseUID, config) {
    ...
    executeBehaviorActions(baseUID, enemyData) {
        const enemy = getEnemyInstance(baseUID, this.runtime);
        if (!enemy)
            return;
    ...
    executeAction(baseUID, enemyData, action) {
        const enemy = getEnemyInstance(baseUID, this.runtime);
        if (!enemy)
            return;
        switch (action.type) {
    ...
    executeMovementAction(enemy, enemyData, action) {
        try {
            const behavior8Dir = enemy.behaviors?._8Direction || enemy.behaviors?.['8Direction'];
            if (!behavior8Dir)
                return;
    ...
    selectBehavior(behaviors, excludeBehaviors) {
        const availableBehaviors = behaviors.filter((b) => !excludeBehaviors.includes(b.name));
        const totalWeight = availableBehaviors.reduce((sum, behavior) => sum + behavior.weight, 0);
    ...
    checkBehaviorConditions(enemyData, conditions) {
...
export function initializeSystem(runtime) {
    aiFactory.setRuntime(runtime);
    console.log("🎬 Multi-file Enemy AI System initialized!");
}
export function initEnemy(baseUID, maskUID, enemyType) {
