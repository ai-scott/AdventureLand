...
// This C# stub preserves the ItemData.ItemCategory enum so existing
// C# call sites (`item.Category() == ItemData.ItemCategory.Weapon`,
// `Inventory.GetEquipped(ItemData.ItemCategory.Boot)`) keep compiling
// without churning every enum reference in InventoryUI / ItemPickupToast /
// HUD on the same commit that ports ItemData itself. The enum integer
// values mirror the GDScript ItemCategory exactly -- don't reorder
// either side.
//
...
// Extension-method bridge over the GDScript ItemData Resource.
// Lets remaining C# consumers (ItemPickupToast, InventoryUI, HUD)
// keep most of their `item.Foo` ergonomics with a method-call suffix
// (`item.Name()`, `item.Strength()`) instead of rewriting every site
// to `.Get("snake_case").AsX()`.
//
...
public static class ItemDataExt
{
    ...
    public static ItemData.ItemCategory Category(this Resource r)
        => (ItemData.ItemCategory)(r?.Get("category").AsInt32() ?? 0);
    ...
    // Computed funcs on the GDScript side -- call via .Call() not .Get().
