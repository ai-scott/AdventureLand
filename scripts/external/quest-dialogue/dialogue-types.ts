...
export interface DialogueNode {
  ...
  responses?: DialogueResponse[];       // What the player can say back (undefined = auto-advance)
  actions?: DialogueAction[];           // What happens when this dialogue triggers
  priority: number;                     // Priority for sorting dialogue options
  variables?: Record<string, string>;   // For variable substitution like [PlayerName]
  autoAdvance?: string;                 // Auto-advance to this node on click/space (no response buttons)
  endsDialogue?: boolean;               // True if this node ends the dialogue (outcome: "End")
}

export interface DialogueCondition {
  type: 'quest_status' | 'has_item' | 'world_flag' | 'npc_memory' | 'player_level' | 'multiple_quests' | 'custom';
...
export interface DialogueAction {
  ...
  variable?: string;                    // Variable name for input action
...
// Dialogue system result (used by AdventureLandIntegration)
