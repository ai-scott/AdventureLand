...
public partial class EnemyController : CharacterBody2D
{
    ...
    public override void _Ready()
    {
        ...
        // Cache the player reference. Expected in group "player" (Player.tscn should have Add to Group).
    ...
    {
        _isHurt = true;
        // Hurt flash — tween Modulate white briefly. Deferred to stay on main thread.
        var sprite = GetNodeOrNull<CanvasItem>("Sprite2D");
        if (sprite != null)
        {
            sprite.Modulate = new Color(2f, 2f, 2f, 1f);
            var t = CreateTween();
            t.TweenProperty(sprite, "modulate", new Color(1f, 1f, 1f, 1f), 0.15);
        }
